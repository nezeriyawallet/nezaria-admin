...
export async function GET(request: Request) {
  ...
  const [values, analytics, revenue] = await Promise.all([Promise.all(Object.entries(endpoints).map(async ([name, endpoint]) => {
    try {
    ...
    try {
      const response = await fetch(`${baseUrl}/admin/api/users/revenue?_=${fresh}`, {
        headers: { "X-Admin-Key": apiKey, Accept: "application/json" }, cache: "no-store",
      });
      return response.ok ? await response.json() as Record<string, unknown> : {};
    } catch { return {}; }
  })()]);
  return Response.json({ metrics: { ...Object.fromEntries(values), ...analytics, ...revenue }, updatedAt: new Date().toISOString() }, {
