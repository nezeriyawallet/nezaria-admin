...
function MarketingSite({ token, onRefresh, onEnter }: { token: string; onRefresh: () => void; onEnter: () => void }) {
  const faqs = [
    ["Скільки часу займає реєстрація?", "Кілька секунд: відкрийте Nezeriya Wallet, відскануйте QR-код і підтвердьте вхід. Паролі та довгі форми не потрібні."],
    ["Які активи можна приймати?", "У кабінеті можна налаштувати приймання USDT і GRAM для кожного бізнесу окремо."],
    ["Як я отримую гроші?", "Оплати проходять через серверний гаманець Nezeriya для перевірки. Після цього власник виводить доступний баланс на підключений Nezeriya Wallet."],
    ["Що відбувається після 24 годин?", "Сесія завершується для безпеки. Щоб знову відкрити кабінет, достатньо ще раз відсканувати QR-код у Nezeriya Wallet."],
  ];
  return <main className="marketing">
    <header className="marketing-nav"><button className="marketing-logo" type="button" onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })}>NEZERIYA <b>PAY</b></button><nav><a href="#features">Можливості</a><a href="#how">Як це працює</a><a href="#cabinet">Кабінет</a><a href="#about">Про нас</a><a href="#security">Безпека</a><a href="#developers">Розробникам</a><a href="#pricing">Тариф</a><a href="#faq">FAQ</a></nav><div><button type="button" className="marketing-login" onClick={onEnter}>Увійти</button><button type="button" className="marketing-primary" onClick={onEnter}>Реєстрація</button></div></header>
    <section className="marketing-hero"><div className="marketing-wrap hero-grid"><div className="hero-registration"><h1>Реєстрація<br />стала простіше</h1><p className="hero-copy">Безпечна реєстрація через застосунок<br />Nezeriya Wallet.</p><div className="hero-benefits"><span><i><Icon name="bolt" /></i><b>Швидко<small>Усього кілька секунд<br />у застосунку</small></b></span><span><i><Icon name="lock" /></i><b>Безпечно<small>Ваші дані під надійним<br />захистом</small></b></span><span><i><Icon name="phone" /></i><b>Через Nezeriya Wallet<small>Реєстрація в офіційному<br />застосунку</small></b></span></div></div><article id="hero-qr" className="hero-login-card"><h2>Реєстрація акаунта</h2><p className="subtitle">Nezeriya Pay</p><h3>Персональний QR-код для реєстрації</h3><p>Відскануйте QR-код у застосунку Nezeriya Wallet,<br />щоб зареєструватися.</p><Qr token={token} /><div className="notice"><Icon name="info" /><span>QR-код одноразовий та прив’язаний до вашої сесії.<br />Після успішної реєстрації він стане неактивним.</span></div><div className="qr-controls"><button type="button" onClick={onRefresh}><Icon name="refresh" /><span>Оновити QR</span></button></div><div className="secure"><Icon name="shield" /><span>Реєстрація захищена сучасними технологіями шифрування.<br />Ваші дані залишаються тільки у вашому гаманці.</span></div><small>Сесія триває 24 години, потім QR потрібен знову.</small></article></div></section>
    <section id="features" className="marketing-section"><div className="marketing-wrap"><p className="eyebrow">МОЖЛИВОСТІ</p><h2>Три способи приймати оплату в крипті</h2><p className="section-lead">Всі надходження перевіряються на серверному гаманці Nezeriya. Кошти для виведення доступні у вашому кабінеті.</p><div className="feature-grid"><article><div className="feature-art terminal-art">▦<span>QR</span></div><h3>Термінал</h3><p>Каса на місці з QR-кодом для оплати в Nezeriya Wallet.</p><ul><li>Квадратний екран каси</li><li>Каталог товарів</li><li>Автоматичні чеки</li></ul></article><article><div className="feature-art link-art"><small>Ваше замовлення</small><b>Оплатити за посиланням</b></div><h3>Платіжні посилання</h3><p>Створюйте рахунки з каталогу або на власну суму.</p><ul><li>Термін дії та одноразовість</li><li>Дійсний QR-код</li><li>Статус оплати в кабінеті</li></ul></article><article><div className="feature-art site-art"><span>POST /payments</span><b>Оплатити</b></div><h3>Сайт</h3><p>Приймання замовлень з вашого сайту у єдиній історії платежів.</p><ul><li>Єдиний облік платежів</li><li>Статуси та чеки</li><li>Баланс бізнесу</li></ul></article></div></div></section>
    <section id="how" className="marketing-section muted"><div className="marketing-wrap"><p className="eyebrow">ЯК ЦЕ ПРАЦЮЄ</p><h2>Від входу до виплати — у чотири кроки</h2><div className="steps-grid">{[["1", "Вхід через QR", "Скануєте QR у Nezeriya Wallet — без пароля."], ["2", "Бізнес і товари", "Додаєте бізнес та каталог із фотографіями."], ["3", "Приймання оплати", "Створюєте термінал або платіжне посилання."], ["4", "Виплата", "Виводите доступні кошти на свій Nezeriya Wallet."]].map(([number, title, text]) => <article key={number}><b>{number}</b><h3>{title}</h3><p>{text}</p></article>)}</div><div className="metric-grid"><article><b>0,5%</b><span>комісія за платіж</span></article><article><b>15 хв</b><span>типовий термін дії рахунку</span></article><article><b>USDT · GRAM</b><span>підтримувані активи</span></article><article><b>24/7</b><span>приймання платежів</span></article></div></div></section>
    <section id="cabinet" className="marketing-section"><div className="marketing-wrap cabinet-section"><div><p className="eyebrow">КАБІНЕТ БІЗНЕСУ</p><h2>Усе про ваш бізнес в одному кабінеті</h2><ul className="cabinet-list"><li><i>↗</i><span><b>Аналітика</b><small>Оборот, чек та статуси платежів</small></span></li><li><i>▧</i><span><b>Товари</b><small>Каталог із квадратними фото</small></span></li><li><i>↓</i><span><b>Виплати</b><small>На підключений Nezeriya Wallet</small></span></li><li><i>⌘</i><span><b>Термінали і посилання</b><small>Каси, QR та рахунки для клієнтів</small></span></li></ul><button type="button" className="marketing-primary" onClick={onEnter}>Відкрити кабінет</button></div><div className="cabinet-demo"><header><span>● ● ●</span><small>nezeriya.pay/cabinet</small></header><div className="demo-body"><div className="demo-head"><b>Кав'ярня</b><em>Активний</em></div><div className="demo-stats"><article><small>Обіг</small><b>0,00 USDT</b></article><article><small>Платежів</small><b>0</b></article><article><small>Середній чек</small><b>0,00 USDT</b></article></div><div className="demo-chart"><span>Оборот за 30 днів</span><i /><i /><i /><i /><i /><i /></div><div className="demo-bottom"><article><b>Топ товарів</b><span>Дані з'являться після оплати</span></article><article><b>Статуси платежів</b><span>Чеки записуються автоматично</span></article></div></div></div></div></section>
    <section id="developers" className="marketing-section developer-section"><div className="marketing-wrap developer-grid"><div><p className="eyebrow">РОЗРОБНИКАМ</p><h2>Інтегруйте приймання оплати у свій продукт</h2><p>Створюйте платіжні сторінки, відстежуйте статуси та ведіть усі операції у кабінеті бізнесу.</p><ul><li>Платіжні посилання і QR-коди</li><li>Захищена QR-авторизація</li><li>Єдина історія чеків і виплат</li></ul><button type="button" className="marketing-outline" onClick={onEnter}>Створити бізнес</button></div><pre><code><span>POST</span> /v1/payments{`\n`}{`{\n  "amount": "6.70",\n  "asset": "USDT",\n  "source": "payment_link"\n}`}<br /><br /><b>200 Payment created</b></code></pre></div></section>
    <section id="pricing" className="marketing-section"><div className="marketing-wrap pricing-grid"><article className="price-card"><p className="eyebrow">ТАРИФ</p><h2>Одна комісія</h2><strong>0,5%</strong><p>від суми успішного платежу</p><ul><li>Без щомісячної плати</li><li>Платежі в USDT і GRAM</li><li>Термінали, посилання та сайт</li><li>Виплата на Nezeriya Wallet</li></ul><button type="button" onClick={onEnter}>Почати</button></article><div id="faq" className="faq"><p className="eyebrow">FAQ</p>{faqs.map(([question, answer], index) => <details key={question} open={index === 0}><summary>{question}<span>+</span></summary><p>{answer}</p></details>)}</div></div></section>
    <section id="about" className="marketing-section about-section"><div className="marketing-wrap"><p className="eyebrow">ПРО НАС</p><h2>Крипто-еквайринг, який зрозуміє будь-який бізнес</h2><p className="section-lead">Nezeriya Pay допомагає приймати оплату в USDT і GRAM без складних рахунків: через касу, посилання або сайт.</p><div className="about-metrics"><article><b>0,5%</b><span>єдина комісія</span></article><article><b>USDT · GRAM</b><span>активи для оплати</span></article><article><b>24/7</b><span>приймання платежів</span></article><article><b>QR</b><span>вхід і підтвердження</span></article></div><h3 className="about-title">Наші принципи</h3><div className="about-principles"><article><b>Прозорість</b><p>Комісія відображається в кожному чеку до зарахування на баланс.</p></article><article><b>Гроші під вашим контролем</b><p>Надходження перевіряються, а доступний баланс виводиться на ваш Nezeriya Wallet.</p></article><article><b>Швидкий старт</b><p>Зареєструйтесь QR-кодом, створіть бізнес та сформуйте перше посилання.</p></article></div></div></section>
    <section id="security" className="marketing-section security-section"><div className="marketing-wrap security-grid"><div><p className="eyebrow">БЕЗПЕКА</p><h2>Кожен чек має зрозумілу перевірку</h2><p className="section-lead">У деталях платежу видно склад замовлення, суму, комісію, статус, хеш транзакції та гаманець клієнта.</p><button type="button" className="marketing-primary" onClick={onEnter}>Відкрити кабінет</button></div><div className="security-cards"><article><b>01</b><h3>Перевірка суми</h3><p>Статус показує, чи отримано всю суму та чи зараховано її на баланс.</p></article><article><b>02</b><h3>Хеш у чеку</h3><p>Після мережевого підтвердження хеш зберігається разом з операцією.</p></article><article><b>03</b><h3>Деталі товарів</h3><p>У квитанції відображаються позиції, кількість і вартість кожної.</p></article></div></div></section>
    <footer className="marketing-footer"><div className="marketing-wrap"><b>NEZERIYA <i>PAY</i></b><span>Приймання платежів у USDT і GRAM для бізнесу.</span><button type="button" onClick={onEnter}>Увійти через QR</button></div></footer>
  </main>;
}
...
function PaymentsPanel({ payments, onGoTerminals }: { payments: Payment[]; onGoTerminals: () => void }) {
  const [query, setQuery] = useState("");
  const [status, setStatus] = useState("Усі");
  const [source, setSource] = useState("Усі");
  const [selected, setSelected] = useState<Payment | null>(null);
  const filtered = payments.filter((payment) => (status === "Усі" || payment.status === status) && (source === "Усі" || payment.source === source) && `${payment.id} ${payment.sourceName} ${payment.products.map((item) => item.name).join(" ")} ${payment.transaction || ""}`.toLocaleLowerCase("uk-UA").includes(query.trim().toLocaleLowerCase("uk-UA")));
  const total = (payment: Payment) => payment.products.length ? payment.products.map((item) => item.name).join(", ") : "Оплата без товарів";
  const formatDate = (value: string) => new Intl.DateTimeFormat("uk-UA", { day: "2-digit", month: "2-digit", year: "numeric", hour: "2-digit", minute: "2-digit" }).format(new Date(value));
  const statusClass = (payment: Payment) => payment.status === "Оплачено" ? "paid" : payment.status === "Недоплата" ? "underpaid" : "pending";
  const verificationText = (payment: Payment) => payment.status === "Оплачено" ? "Оплату перевірено та зараховано" : payment.status === "Недоплата" ? "Надійшла не вся сума — потрібна доплата" : "Очікуємо підтвердження у мережі";
  useEffect(() => {
    if (!selected) return;
  ...
  if (!payments.length) return <section className="payments-view"><header className="payments-heading"><h2>Платежі</h2><p>Усі оплати від клієнтів: з терміналів, платіжних посилань і сайту</p></header><div className="payments-empty"><i>▤</i><h3>Платежів поки немає</h3><p>Чеки з’являються автоматично після успішної оплати через термінал, платіжне посилання або сайт.</p><button type="button" onClick={onGoTerminals}>▦&nbsp; Перейти до терміналів</button><div className="payment-sources"><article><b>▦</b><div><strong>Термінал</strong><small>Клієнт платить за QR-кодом на касі</small></div><em>→&nbsp; Чек автоматично потрапляє в «Платежі»</em></article><article><b>↗</b><div><strong>Платіжне посилання</strong><small>Клієнт платить за посиланням, яке ви надіслали</small></div><em>→&nbsp; Чек автоматично потрапляє в «Платежі»</em></article><article><b>⌘</b><div><strong>Сайт</strong><small>Оплата замовлення через інтеграцію</small></div><em>→&nbsp; Чек автоматично потрапляє в «Платежі»</em></article></div></div></section>;
  return <section className="payments-view"><header className="payments-heading"><h2>Платежі</h2><p>{payments.length} чеків із терміналів, платіжних посилань і сайту</p></header><div className="payments-layout"><div className="payments-tools"><label>⌕<input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Пошук за ID, товаром або хешем" /></label><select value={status} onChange={(event) => setStatus(event.target.value)}><option>Усі</option><option>Оплачено</option><option>Очікує підтвердження</option><option>Недоплата</option></select><select value={source} onChange={(event) => setSource(event.target.value)}><option>Усі</option><option>Термінал</option><option>Платіжне посилання</option><option>Сайт</option></select></div><div className="payments-table"><div className="payments-table-head"><span>Дата і час</span><span>Платіж</span><span>Джерело</span><span>Сума</span><span>Статус</span></div>{filtered.map((payment) => <button type="button" className={selected?.id === payment.id ? "receipt-row selected" : "receipt-row"} key={payment.id} onClick={() => setSelected(payment)}><span><b>{formatDate(payment.createdAt).split(",")[0]}</b><small>{formatDate(payment.createdAt).split(",")[1]?.trim()}</small></span><span><b>{total(payment)}</b><small>{payment.id} · {payment.products.length} {payment.products.length === 1 ? "товар" : "товари"}</small></span><span>{payment.source === "Термінал" ? "▦" : payment.source === "Сайт" ? "⌘" : "↗"} {payment.sourceName}</span><strong>+{payment.amount} {payment.currency}</strong><em className={statusClass(payment)}>{payment.status}</em></button>)}{!filtered.length && <p className="payments-no-results">Чеків за цими фільтрами немає.</p>}</div></div>{selected && <div className="receipt-drawer-layer" role="presentation" onMouseDown={() => setSelected(null)}><aside className="receipt-detail" role="dialog" aria-modal="true" aria-labelledby="receipt-title" onMouseDown={(event) => event.stopPropagation()}><button type="button" aria-label="Закрити деталі платежу" onClick={() => setSelected(null)}>×</button><p className="receipt-kicker">Деталі платежу</p><h2 id="receipt-title">Платіж {selected.id}</h2><p><em className={statusClass(selected)}>{selected.status}</em> {formatDate(selected.createdAt)}</p><strong>{selected.amount} {selected.currency}</strong><div className={`receipt-verification ${statusClass(selected)}`}><b>{selected.status === "Оплачено" ? "✓" : "!"}</b><span><strong>{verificationText(selected)}</strong><small>{selected.status === "Оплачено" ? "Сума та реквізити звірені" : "Статус оновиться після підтвердження"}</small></span></div><small>Джерело</small><h3>{selected.source === "Термінал" ? "▦" : selected.source === "Сайт" ? "⌘" : "↗"} {selected.sourceName}</h3><div className="receipt-products"><b>Товари ({selected.products.length})</b>{selected.products.length ? selected.products.map((item, index) => <article key={`${item.name}-${index}`}>{item.photo ? <img src={item.photo} alt="" /> : <i>▧</i>}<span><strong>{item.name}</strong><small>× {item.quantity}</small></span><b>{item.price} {item.currency}</b></article>) : <p>До платежу не додано товари.</p>}</div><dl><div><dt>Разом</dt><dd>{selected.amount} {selected.currency}</dd></div><div><dt>Комісія 0,5%</dt><dd>−{(Number(selected.amount) * .005).toFixed(2)} {selected.currency}</dd></div><div><dt>Зараховано на баланс</dt><dd>{(Number(selected.amount) * .995).toFixed(2)} {selected.currency}</dd></div></dl><footer><small>Хеш транзакції</small><b>{selected.transaction || "З’явиться після підтвердження"}</b>{selected.wallet && <><small>Гаманець клієнта</small><b>{selected.wallet}</b></>}<small>Мережа</small><b>TON</b></footer></aside></div>}</section>;
}

function PayoutsPanel({ account, business, payments, payouts, wallet, session, assets, onCreate }: { account: string; business: string; payments: Payment[]; payouts: Payout[]; wallet: string; session: string; assets: string[]; onCreate: (payout: Payout) => void }) {
  const [asset, setAsset] = useState<"USDT" | "GRAM">(assets.includes("USDT") ? "USDT" : "GRAM");
  const [amount, setAmount] = useState("");
  const [open, setOpen] = useState(false);
  const [filter, setFilter] = useState("Усі");
  const amountFor = (items: Array<Payment | Payout>, currency: "USDT" | "GRAM", predicate?: (item: Payment | Payout) => boolean) => items.filter((item) => item.currency === currency && (!predicate || predicate(item))).reduce((sum, item) => sum + (Number(item.amount) || 0), 0);
  const available = (currency: "USDT" | "GRAM") => Math.max(0, amountFor(payments, currency, (item) => (item as Payment).status === "Оплачено") * .995 - amountFor(payouts, currency));
  const format = (value: number) => value.toLocaleString("uk-UA", { minimumFractionDigits: 2, maximumFractionDigits: 2 });
  const visible = payouts.filter((item) => filter === "Усі" || item.currency === filter);
  const numeric = Number(amount.replace(",", "."));
  const fee = 0;
  const valid = Boolean(wallet) && numeric > 0 && numeric <= available(asset) && numeric > fee;
  const openWithdraw = () => { setAmount(""); setOpen(true); };
  const create = async (event: FormEvent) => { event.preventDefault(); if (!valid) return; const response = await fetch("/api/acquiring/payout", { method: "POST", headers: { "Content-Type": "application/json", "x-acquiring-session": session }, body: JSON.stringify({ account, business, amount: numeric.toFixed(2), currency: asset, wallet }) }); const data = await response.json().catch(() => ({})) as { payout?: Payout; error?: string }; if (!response.ok || !data.payout) { window.alert(data.error || "Не вдалося виконати виплату"); return; } onCreate(data.payout); setAmount(""); setOpen(false); };
...
function BusinessWorkspace({ business, account, onBack, products, payments, payouts, links, payoutWallet, payoutSession, onLinksChange, onPayoutsChange, onProductsChange, onBusinessChange, onDelete }: { business: Business; account: string; onBack: () => void; products: Product[]; payments: Payment[]; payouts: Payout[]; links: PaymentLink[]; payoutWallet: string; payoutSession: string; onLinksChange: (links: PaymentLink[]) => void; onPayoutsChange: (payouts: Payout[]) => void; onProductsChange: (products: Product[]) => void; onBusinessChange: (business: Business) => void; onDelete: () => void }) {
  ...
  const tabs = ["Аналітика", "Платежі", "Термінали", "Посилання", "Товари", "Виплати", "Розробникам", "Налаштування"];
  const analyticsDays: Record<string, number> = { "7 днів": 7, "30 днів": 30, "Квартал": 90, "Свій період": 30 };
  const dayKey = (date: Date) => `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, "0")}-${String(date.getDate()).padStart(2, "0")}`;
  const money = (value: number) => value.toLocaleString("uk-UA", { minimumFractionDigits: 2, maximumFractionDigits: 2 });
  const days = analyticsDays[period];
  const now = new Date();
  ...
  const inRange = (payment: Payment, from: Date, to: Date) => { const timestamp = new Date(payment.createdAt).getTime(); return Number.isFinite(timestamp) && timestamp >= from.getTime() && timestamp <= to.getTime(); };
  const currentPayments = payments.filter((payment) => payment.currency === unit && inRange(payment, currentStart, now));
  const paidPayments = currentPayments.filter((payment) => payment.status === "Оплачено");
  const previousPaid = payments.filter((payment) => payment.currency === unit && payment.status === "Оплачено" && inRange(payment, previousStart, new Date(currentStart.getTime() - 1)));
  const turnover = paidPayments.reduce((sum, payment) => sum + (Number(payment.amount) || 0), 0);
  const previousTurnover = previousPaid.reduce((sum, payment) => sum + (Number(payment.amount) || 0), 0);
  const successRate = currentPayments.length ? paidPayments.length / currentPayments.length * 100 : 0;
  const trend = (current: number, previous: number) => previous > 0 ? `${current >= previous ? "+" : ""}${((current - previous) / previous * 100).toFixed(1)}%` : current > 0 ? "+100%" : "—";
  const dailyTurnover = Array.from({ length: days }, (_, index) => {
    const date = new Date(currentStart);
    date.setDate(date.getDate() + index);
    const key = dayKey(date);
    return { key, label: new Intl.DateTimeFormat("uk-UA", { day: "2-digit", month: "2-digit" }).format(date), total: paidPayments.filter((payment) => dayKey(new Date(payment.createdAt)) === key).reduce((sum, payment) => sum + (Number(payment.amount) || 0), 0) };
  });
  const notify = (text: string) => { setMessage(text); window.setTimeout(() => setMessage(""), 2200); };
  const exportAnalytics = () => { const data = ["Бізнес,Період,Актив,Дата,Оплачено", ...dailyTurnover.map((item) => `${business.name},${period},${unit},${item.key},${item.total.toFixed(2)}`), `Разом,${period},${unit},,${turnover.toFixed(2)}`].join("\n"); const url = URL.createObjectURL(new Blob([data], { type: "text/csv" })); const link = document.createElement("a"); link.href = url; link.download = `${business.name}-analytics.csv`; link.click(); URL.revokeObjectURL(url); notify("Файл експорту сформовано"); };
  ...
  return <main className="business-workspace">
    ...
    <section className="workspace-shell"><button className="back-businesses" onClick={onBack}>‹ Мої бізнеси</button><div className="workspace-title"><div className="workspace-business-mark">{business.logo ? <img src={business.logo} alt={`Логотип ${business.name}`} style={{ width: "100%", height: "100%", borderRadius: "inherit", objectFit: "cover" }} /> : "▣"}</div><div><h1>{business.name}</h1><p><span className={business.suspended ? "status-paused" : "status-active"}>{business.suspended ? "Призупинений" : "Активний"}</span>{business.ownership} {business.owner && ` ${business.owner}`} · {business.type} · {business.assets?.join(", ") || "USDT"}</p></div><div className="workspace-title-actions"><button onClick={() => setDetailsOpen(true)}>Дані бізнесу</button><button aria-label="Додаткові дії" className="more-button" onClick={() => setMenuOpen(!menuOpen)}>•••</button>{menuOpen && <div className="workspace-popover actions-popover"><button onClick={copyBusinessId}>Копіювати ID</button><button onClick={() => { setMenuOpen(false); notify("Бізнес активний"); }}>Перевірити статус</button></div>}</div></div>
      ...
      {tab === "Аналітика" ? <section className="analytics-view"><div className="analytics-filters"><div>{["7 днів", "30 днів", "Квартал", "Свій період"].map((item) => <button key={item} className={period === item ? "chosen" : ""} onClick={() => setPeriod(item)}>{item}</button>)}</div><div>{["Усі", "USDT", "GRAM"].filter((item) => item === "Усі" || business.assets?.includes(item)).map((item) => <button key={item} className={asset === item ? "chosen light" : ""} onClick={() => setAsset(item)}>{item}</button>)}</div><button className="export-button" onClick={exportAnalytics}>⇩ Експорт</button></div>
        <div className="metric-grid"><Metric label="Обіг" value={`${money(turnover)} ${unit}`} trend={trend(turnover, previousTurnover)} negative={turnover < previousTurnover} /><Metric label="Платежів" value={String(paidPayments.length)} trend={trend(paidPayments.length, previousPaid.length)} negative={paidPayments.length < previousPaid.length} /><Metric label="Середній чек" value={`${money(paidPayments.length ? turnover / paidPayments.length : 0)} ${unit}`} trend="—" /><Metric label="Успішних платежів" value={`${successRate.toFixed(1)}%`} trend="—" /></div>
        <div className="analytics-main"><Chart period={period} unit={unit} daily={dailyTurnover} /><TerminalStats unit={unit} payments={paidPayments} /></div><div className="analytics-bottom"><TopItems unit={unit} payments={paidPayments} /><PaymentStatuses payments={currentPayments} /><AssetStats assets={business.assets || ["USDT"]} payments={payments.filter((payment) => inRange(payment, currentStart, now) && payment.status === "Оплачено")} /></div>
      </section> : tab === "Платежі" ? <PaymentsPanel payments={payments} onGoTerminals={() => setTab("Термінали")} /> : tab === "Посилання" ? <PaymentLinksPanel business={business} products={products} links={links} onChange={onLinksChange} notify={notify} /> : tab === "Виплати" ? <PayoutsPanel account={account} business={business.name} payments={payments} payouts={payouts} wallet={payoutWallet} session={payoutSession} assets={business.assets || ["USDT"]} onCreate={(payout) => { onPayoutsChange([payout, ...payouts]); notify("Виплату виконано"); }} /> : tab === "Товари" ? (productAddOpen || editingProduct) ? <ProductForm initialProduct={editingProduct || undefined} onCancel={() => { setProductAddOpen(false); setEditingProduct(null); }} onSave={(product) => { if (editingProduct) { onProductsChange(products.map((item) => item === editingProduct ? product : item)); notify("Товар оновлено"); } else { onProductsChange([...products, product]); } setProductAddOpen(false); setEditingProduct(null); }} /> : <ProductCatalog products={products} onAdd={() => { setEditingProduct(null); setProductAddOpen(true); }} onEdit={(product) => setEditingProduct(product)} onRemove={(target) => { onProductsChange(products.filter((product) => product !== target)); notify("Товар видалено"); }} onImport={() => notify("Імпорт CSV відкрито")} /> : tab === "Налаштування" ? <BusinessSettings business={business} onSave={onBusinessChange} onDelete={onDelete} notify={notify} /> : <section className="workspace-empty"><h2>{tab}</h2><p>{tabActions[tab][1]}</p><button onClick={() => notify(`${tabActions[tab][0]} — функція відкрита`)}>{tabActions[tab][0]}</button></section>}
...
function Chart({ period, unit, daily }: { period: string; unit: string; daily: Array<{ key: string; label: string; total: number }> }) { const max = Math.max(...daily.map((item) => item.total), 1); const step = 760 / Math.max(daily.length, 1); const barWidth = Math.max(3, Math.min(22, step - 3)); const labels = daily.filter((_, index) => index === 0 || index === daily.length - 1 || index % Math.max(1, Math.ceil(daily.length / 6)) === 0); return <article className="chart-card"><h2>Обіг за {period}</h2><span>● Обіг, {unit}</span><svg viewBox="0 0 800 200" role="img" aria-label="Стовпчиковий графік обігу"><line className="chart-baseline" x1="20" y1="170" x2="780" y2="170" />{daily.map((item, index) => { const height = item.total ? Math.max(3, item.total / max * 136) : 0; const x = 20 + index * step + (step - barWidth) / 2; return <g key={item.key}><title>{item.label}: {item.total.toLocaleString("uk-UA", { minimumFractionDigits: 2, maximumFractionDigits: 2 })} {unit}</title>{height > 0 && <rect className="chart-bar" x={x} y={170 - height} width={barWidth} height={height} rx="3" />}</g>; })}</svg>{daily.every((item) => item.total === 0) && <p className="chart-empty">Оплачених платежів за цей період ще немає</p>}<div className="chart-labels">{labels.map((item) => <span key={item.key}>{item.label}</span>)}</div></article>; }
function TerminalStats({ unit, payments }: { unit: string; payments: Payment[] }) { const totals = new Map<string, number>(); payments.forEach((payment) => totals.set(payment.sourceName || payment.source, (totals.get(payment.sourceName || payment.source) || 0) + (Number(payment.amount) || 0))); const rows = [...totals.entries()].sort((a, b) => b[1] - a[1]).slice(0, 4); const max = Math.max(...rows.map(([, value]) => value), 1); return <article className="terminal-card"><h2>За терміналами</h2>{rows.length ? rows.map(([name, value]) => <div key={name}><p><span>{name}</span><b>{value.toLocaleString("uk-UA", { minimumFractionDigits: 2, maximumFractionDigits: 2 })} {unit}</b></p><i><em style={{ width: `${value / max * 100}%` }} /></i></div>) : <p className="analytics-empty">Оплачених платежів ще немає.</p>}</article>; }
function TopItems({ unit, payments }: { unit: string; payments: Payment[] }) { const items = new Map<string, { quantity: number; amount: number }>(); payments.flatMap((payment) => payment.products).forEach((item) => { const current = items.get(item.name) || { quantity: 0, amount: 0 }; current.quantity += item.quantity || 0; current.amount += (Number(item.price) || 0) * (item.quantity || 0); items.set(item.name, current); }); const rows = [...items.entries()].sort((a, b) => b[1].amount - a[1].amount).slice(0, 3); return <article className="mini-card"><h2>Топ товарів</h2>{rows.length ? rows.map(([name, item], index) => <p key={name}><i>{index + 1}</i><span>{name}<small>{item.quantity} шт.</small></span><b>{item.amount.toLocaleString("uk-UA", { minimumFractionDigits: 2, maximumFractionDigits: 2 })} {unit}</b></p>) : <p className="analytics-empty">Продажів товарів ще немає.</p>}</article>; }
function PaymentStatuses({ payments }: { payments: Payment[] }) { const statuses: Array<[Payment["status"], string, string]> = [["Оплачено", "Оплачено", "#1eae68"], ["Очікує підтвердження", "Очікує", "#f0a92a"], ["Недоплата", "Недоплата", "#ef5b64"]]; const total = payments.length || 1; const value = (status: Payment["status"]) => payments.filter((item) => item.status === status).length / total * 100; return <article className="mini-card"><h2>Статуси платежів</h2><div className="status-bar">{statuses.map(([status, , color]) => <i key={status} style={{ width: `${value(status)}%`, background: color }} />)}</div><div className="status-list">{statuses.map(([status, label]) => <p key={status}><span>● {label}</span><b>{value(status).toFixed(1)}%</b></p>)}</div></article>; }
function AssetStats({ assets, payments }: { assets: string[]; payments: Payment[] }) { const totals = assets.map((asset) => ({ asset, total: payments.filter((payment) => payment.currency === asset).reduce((sum, payment) => sum + (Number(payment.amount) || 0), 0) })); const total = totals.reduce((sum, item) => sum + item.total, 0); return <article className="mini-card assets-card"><h2>Активи</h2><div className="asset-bar">{totals.map((item) => <i key={item.asset} style={{ width: `${total ? item.total / total * 100 : 0}%` }} />)}</div>{totals.map((item) => <p key={item.asset}><span>● {item.asset} · {total ? (item.total / total * 100).toFixed(0) : 0}%</span><b>{item.total.toLocaleString("uk-UA", { minimumFractionDigits: 2, maximumFractionDigits: 2 })} {item.asset}</b></p>)}</article>; }

function PaymentLinksPanel({ business, products, links, onChange, notify }: { business: Business; products: Product[]; links: PaymentLink[]; onChange: (links: PaymentLink[]) => void; notify: (text: string) => void }) { const [open, setOpen] = useState(false); const [query, setQuery] = useState(""); const now = Date.now(); const visible = links.map((link) => link.status === "Активне" && new Date(link.expiresAt).getTime() < now ? { ...link, status: "Прострочено" as const } : link).filter((link) => `${link.title} ${link.id}`.toLowerCase().includes(query.toLowerCase())); const copy = async (link: PaymentLink) => { await navigator.clipboard?.writeText(`${window.location.origin}/pay/${link.id}`); notify("Посилання скопійовано"); }; return <section className="links-page"><header><div><h2>Платіжні посилання</h2><p>Оплата без каси: надішліть посилання клієнту в чат або SMS</p></div><button onClick={() => setOpen(true)}>⊕&nbsp; Створити посилання</button></header><div className="links-tools"><b>Усі {links.length}</b><label>⌕<input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Пошук за товаром або ID" /></label></div><div className="links-table"><div className="links-head"><span>Посилання</span><span>Сума</span><span>Створено</span><span>Діє до</span><span>Статус</span><span>Дії</span></div>{visible.length ? visible.map((link) => <div className="link-row" key={link.id}><span><b>{link.title}</b><small>{link.id} · {link.products.length ? `${link.products.length} товар(и)` : "Своя сума"}{link.note ? ` · ${link.note}` : ""}</small></span><b>{link.amount} {link.currency}</b><span>{new Date(link.createdAt).toLocaleString("uk-UA")}</span><span>{new Date(link.expiresAt).toLocaleString("uk-UA")}</span><em className={link.status === "Оплачено" ? "paid" : link.status === "Прострочено" ? "expired" : "active-link"}>{link.status}</em><span><button onClick={() => copy(link)}>Копіювати</button><button onClick={() => window.open(`${window.location.origin}/pay/${link.id}`, "_blank", "noopener")}>QR</button></span></div>) : <p className="links-empty">Посилань ще немає. Створіть перше для оплати з каталогу.</p>}</div>{open && <PaymentLinkWizard business={business} products={products} onClose={() => setOpen(false)} onCreate={(link) => { onChange([link, ...links]); setOpen(false); notify("Посилання створено"); }} />}</section>; }

function PaymentLinkWizard({ business, products, onClose, onCreate }: { business: Business; products: Product[]; onClose: () => void; onCreate: (link: PaymentLink) => void }) { const [step, setStep] = useState<1 | 2 | 3>(1); const [chosen, setChosen] = useState<Record<string, number>>({}); const [assets, setAssets] = useState<Array<"USDT" | "GRAM">>((business.assets.filter((asset): asset is "USDT" | "GRAM" => asset === "USDT" || asset === "GRAM") || ["USDT"])); const [duration, setDuration] = useState("24 години"); const [oneTime, setOneTime] = useState(true); const [note, setNote] = useState(""); const [message, setMessage] = useState("Дякуємо за замовлення!"); const [created, setCreated] = useState<PaymentLink | null>(null); const selected = products.filter((product) => chosen[product.sku || product.name]); const currency = selected[0]?.currency || assets[0] || "USDT"; const total = selected.reduce((sum, product) => sum + (Number(product.price) || 0) * chosen[product.sku || product.name], 0); const create = () => { const id = `L-${Date.now().toString().slice(-6)}`; const hours = duration === "1 година" ? 1 : duration === "7 днів" ? 168 : 24; const lines = selected.map((product) => ({ name: product.name, quantity: chosen[product.sku || product.name], price: product.price, currency: product.currency, photo: product.photo })); const link: PaymentLink = { id, createdAt: new Date().toISOString(), expiresAt: new Date(Date.now() + hours * 3600000).toISOString(), title: lines.map((line) => line.name).join(", "), amount: total.toFixed(2), currency, assets, products: lines, note, message, oneTime, status: "Активне" }; onCreate(link); setCreated(link); setStep(3); }; const url = created ? `${window.location.origin}/pay/${created.id}` : ""; return <div className="link-backdrop" onMouseDown={onClose}><section className="link-wizard" role="dialog" aria-modal="true" onMouseDown={(event) => event.stopPropagation()}><button className="link-close" onClick={onClose}>×</button><h2>{step === 3 ? "Посилання створено" : "Нове платіжне посилання"}</h2><p>Крок {step} з 3 · {step === 1 ? "Оберіть товари" : step === 2 ? "Параметри" : "Готово"}</p><div className="link-steps"><b className={step >= 1 ? "done" : ""}>1 Товари</b><b className={step >= 2 ? "done" : ""}>2 Параметри</b><b className={step >= 3 ? "done" : ""}>3 Готово</b></div>{step === 1 && <div className="link-content"><div className="link-products">{products.length ? products.map((product) => { const key = product.sku || product.name; const quantity = chosen[key] || 0; return <article key={key} className={quantity ? "selected" : ""}><button onClick={() => setChosen((current) => ({ ...current, [key]: quantity ? 0 : 1 }))}>{quantity ? "✓" : "○"}</button>{product.photo ? <img src={product.photo} alt="" /> : <i>▧</i>}<span><b>{product.name}</b><small>{product.price} {product.currency}</small></span>{quantity > 0 && <div><button onClick={() => setChosen((current) => ({ ...current, [key]: Math.max(1, quantity - 1) }))}>−</button><b>{quantity}</b><button onClick={() => setChosen((current) => ({ ...current, [key]: quantity + 1 }))}>+</button></div>}</article>; }) : <p>У каталозі ще немає товарів. Додайте товар, щоб створити посилання.</p>}</div><aside><h3>У посиланні</h3>{selected.map((product) => <p key={product.name}>{product.name} × {chosen[product.sku || product.name]}</p>)}<strong>{total.toFixed(2)} {currency}</strong></aside></div>}{step === 2 && <div className="link-content parameters"><div><h3>Активи для оплати</h3><div className="link-options">{business.assets.map((asset) => <button key={asset} className={assets.includes(asset as "USDT" | "GRAM") ? "selected" : ""} onClick={() => setAssets((current) => current.includes(asset as "USDT" | "GRAM") ? (current.length > 1 ? current.filter((item) => item !== asset) : current) : [...current, asset as "USDT" | "GRAM"])}>{assets.includes(asset as "USDT" | "GRAM") ? "✓ " : ""}{asset}</button>)}</div><h3>Діє до</h3><div className="link-options">{["1 година", "24 години", "7 днів"].map((item) => <button key={item} className={duration === item ? "selected" : ""} onClick={() => setDuration(item)}>{item}</button>)}</div><label className="switch-row">Одноразове посилання <input type="checkbox" checked={oneTime} onChange={(event) => setOneTime(event.target.checked)} /></label><label>Позначка для себе<input value={note} onChange={(event) => setNote(event.target.value)} /></label><label>Повідомлення покупцю<textarea maxLength={120} value={message} onChange={(event) => setMessage(event.target.value)} /></label></div><aside><h3>Підсумок</h3><p>{selected.map((product) => product.name).join(", ")}</p><strong>{total.toFixed(2)} {currency}</strong><p>{assets.join(", ")} · {duration}</p></aside></div>}{step === 3 && <div className="link-success"><input readOnly value={url} /><div className="link-qr"><img src={`https://api.qrserver.com/v1/create-qr-code/?format=svg&size=250x250&margin=10&data=${encodeURIComponent(url)}`} alt="QR-код платіжного посилання" /></div><p>Покупець може відсканувати QR або відкрити посилання.</p></div>}<footer>{step === 1 ? <><button onClick={onClose}>Скасувати</button><button disabled={!selected.length} onClick={() => setStep(2)}>Далі</button></> : step === 2 ? <><button onClick={() => setStep(1)}>Назад</button><button onClick={create}>Створити посилання</button></> : <button onClick={onClose}>Готово</button>}</footer></section></div>; }
...
export default function AcquiringPage() {
  ...
  const [paymentsByBusiness, setPaymentsByBusiness] = useState<Record<string, Payment[]>>({});
  const [payoutsByBusiness, setPayoutsByBusiness] = useState<Record<string, Payout[]>>({});
  const [linksByBusiness, setLinksByBusiness] = useState<Record<string, PaymentLink[]>>({});
  ...
  useEffect(() => {
    ...
    if (confirmedToken) {
      ...
      finishConnection(connectedAccount, confirmedToken, params.get("wallet") || "");
    ...
    const onConnected = (event: StorageEvent) => {
      if (event.key === "nezeriya_pay_connection") {
        ...
        if (!profile?.walletId || !profile?.session || Number(profile?.expiresAt) <= Date.now()) return;
        const next = profile.name || "Nezeriya Wallet";
        localStorage.setItem("nezeriya_pay_account", next);
        if (profile.walletId) setPayoutWallet(String(profile.walletId)); if (profile.session) setPayoutSession(String(profile.session));
  ...
  useEffect(() => {
    ...
    const checkConnection = async () => {
      try {
        ...
        const result = await response.json() as { connected?: boolean; merchant?: string; wallet?: string; session?: string; expiresAt?: number };
        if (active && result.connected && result.session) finishConnection(result.merchant || "Nezeriya Wallet", token, result.wallet || "", result.session, result.expiresAt);
  ...
  const logout = () => { localStorage.removeItem("nezeriya_pay_account"); localStorage.removeItem("nezeriya_pay_connection"); setPayoutSession(""); setPayoutWallet(""); setToken(makeToken()); setLogoutOpen(false); setView("landing"); window.history.replaceState({}, "", "/acquiring"); };
  const availableBalance = (payments: Payment[], payouts: Payout[], asset: "USDT" | "GRAM") => Math.max(0, payments.filter((item) => item.currency === asset && item.status === "Оплачено").reduce((sum, item) => sum + (Number(item.amount) || 0), 0) * 0.995 - payouts.filter((item) => item.currency === asset).reduce((sum, item) => sum + (Number(item.amount) || 0), 0));
  const allPayments = Object.values(paymentsByBusiness).flat();
  const allPayouts = Object.values(payoutsByBusiness).flat();
  const accountBalance = availableBalance(allPayments, allPayouts, currency);
  ...
  if (view === "dashboard") return <main className="pay-app dashboard">
    ...
    <section className="pay-content"><header><div className="user"><Mark small label={account} /><span><b>{profile.firstName} {profile.lastName}</b><small>Підключено через Wallet</small></span></div></header>
      {dashboardSection === "settings" ? <ProfileSettings profile={profile} businesses={businesses} onCancel={() => setDashboardSection("home")} onSave={(nextProfile) => { setProfile(nextProfile); localStorage.setItem("nezeriya_pay_profile", JSON.stringify(nextProfile)); setDashboardSection("home"); }} /> : <><section className="balance-card"><p>Загальний доступний баланс <i title="Сума підтверджених оплат усіх бізнесів після комісії 0,5% та виплат">i</i></p><h1><span>{formatBalance(accountBalance)}</span><CurrencyMark currency={currency} /></h1><p className="balance-note">Сума всіх бізнесів після комісії еквайрингу 0,5% і виконаних виплат.</p><div className="currencies">{(["USDT", "GRAM"] as const).map((item) => <button key={item} className={currency === item ? "chosen" : ""} onClick={() => setCurrency(item)}>{item}</button>)}</div><button className="withdraw" disabled>Оберіть бізнес для виведення</button></section>
      <section className="businesses-panel">
        <div className="businesses-heading"><h2>Бізнеси</h2></div>
        <div className="business-list">{businesses.map((business) => { const asset = business.assets?.[0] === "GRAM" ? "GRAM" : "USDT"; const balance = availableBalance(paymentsByBusiness[business.name] || [], payoutsByBusiness[business.name] || [], asset); return <button type="button" className="business-card" key={business.name} onClick={() => setSelectedBusiness(business)}>{business.logo ? <img className="business-logo" src={business.logo} alt="" /> : <span>▣</span>}<b>{business.name}</b><small>{business.type}</small><em>Доступно до виведення<strong>{formatBalance(balance)} {asset}</strong></em></button>; })}<button className="add-business-card" onClick={openBusinessForm}><span>＋</span><b>Додати бізнес</b><small>Створіть перший профіль еквайрингу</small></button></div>
      </section></>}
    </section>
