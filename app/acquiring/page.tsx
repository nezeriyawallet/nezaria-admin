...
type Product = { name: string; category: string; sku: string; price: string; currency: "USDT" | "GRAM"; quantity: string; untracked: boolean; description: string; terminals: string[]; photo?: string };
...
function BusinessWorkspace({ business, account, onBack }: { business: Business; account: string; onBack: () => void }) {
  ...
  return <main className="business-workspace">
    ...
    <section className="workspace-shell"><button className="back-businesses" onClick={onBack}>‹ Мої бізнеси</button><div className="workspace-title"><div className="workspace-business-mark">▣</div><div><h1>{business.name}</h1><p><span className="status-active">Активний</span>{business.ownership} {business.owner && ` ${business.owner}`} · {business.type} · {business.assets?.join(", ") || "USDT"}</p></div><div className="workspace-title-actions"><button onClick={() => setDetailsOpen(true)}>Дані бізнесу</button><button aria-label="Додаткові дії" className="more-button" onClick={() => setMenuOpen(!menuOpen)}>•••</button>{menuOpen && <div className="workspace-popover actions-popover"><button onClick={copyBusinessId}>Копіювати ID</button><button onClick={() => { setMenuOpen(false); notify("Бізнес активний"); }}>Перевірити статус</button></div>}</div></div>
      ...
      </section> : tab === "Товари" ? productAddOpen ? <ProductForm onCancel={() => setProductAddOpen(false)} onAdd={(product) => { setProducts((current) => [...current, product]); setProductAddOpen(false); notify("Товар додано"); }} /> : <section className="workspace-empty products-empty"><h2>Товари</h2><p>{products.length ? `Додано товарів: ${products.length}` : "Товари ще не додані"}</p>{products.map((product) => <p className="saved-product" key={`${product.name}-${product.sku}`}>{product.name} — {product.price} {product.currency}</p>)}<button onClick={() => setProductAddOpen(true)}>Додати товар</button></section> : <section className="workspace-empty"><h2>{tab}</h2><p>{tabActions[tab][1]}</p><button onClick={() => notify(`${tabActions[tab][0]} — функція відкрита`)}>{tabActions[tab][0]}</button></section>}
...
function ProductForm({ onCancel, onAdd }: { onCancel: () => void; onAdd: (product: Product) => void }) {
  const [product, setProduct] = useState<Product>({ name: "", category: "", sku: "", price: "", currency: "USDT", quantity: "100", untracked: false, description: "", terminals: ["Каса №1 · Центр", "Каса №2 · Бар"] });
  const terminals = ["Каса №1 · Центр", "Каса №2 · Бар", "Інтернет-магазин"];
  ...
  const submit = (event: FormEvent) => { event.preventDefault(); if (!product.name.trim() || !product.price.trim()) return; onAdd(product); };
  return <section className="product-page"><button type="button" className="product-back" onClick={onCancel}>‹ Товари</button><form className="product-form" onSubmit={submit}><div className="product-form-main"><h2>Додати товар</h2><p>Товар з’явиться на вибраних терміналах і в платіжних посиланнях</p><label className="product-field full"><span>Назва товару <b>*</b></span><input required value={product.name} onChange={(event) => update("name", event.target.value)} placeholder="Наприклад, Капучино 250 мл" /></label><div className="product-row"><label className="product-field"><span>Категорія</span><select value={product.category} onChange={(event) => update("category", event.target.value)}><option value="">Оберіть категорію</option><option>Напої</option><option>Їжа</option><option>Десерти</option><option>Товари</option><option>Послуги</option></select></label><label className="product-field"><span>Артикул (SKU)</span><input value={product.sku} onChange={(event) => update("sku", event.target.value)} placeholder="CAP-250" /></label></div><div className="product-row"><label className="product-field"><span>Ціна <b>*</b></span><div className="price-control"><input required inputMode="decimal" value={product.price} onChange={(event) => update("price", event.target.value)} placeholder="0.00" /><button type="button" className={product.currency === "USDT" ? "active" : ""} onClick={() => update("currency", "USDT")}>USDT</button><button type="button" className={product.currency === "GRAM" ? "active" : ""} onClick={() => update("currency", "GRAM")}>GRAM</button></div></label><label className="product-field"><span>Кількість</span><div className="quantity-control"><input disabled={product.untracked} inputMode="numeric" value={product.quantity} onChange={(event) => update("quantity", event.target.value)} /><button type="button" role="switch" aria-checked={product.untracked} className={product.untracked ? "switch on" : "switch"} onClick={() => update("untracked", !product.untracked)}><i /></button><em>Без обліку</em></div></label></div><label className="product-field full"><span>Опис</span><textarea maxLength={500} value={product.description} onChange={(event) => update("description", event.target.value)} placeholder="Коротко опишіть товар: склад, розмір, особливості" /><small>{product.description.length}/500</small></label><div className="product-terminals"><span>Доступний на терміналах</span><div>{terminals.map((terminal) => <button type="button" key={terminal} className={product.terminals.includes(terminal) ? "selected" : ""} onClick={() => toggleTerminal(terminal)}>{product.terminals.includes(terminal) ? "✓ " : ""}{terminal}</button>)}</div></div></div><aside className="product-preview"><h3>Фото товару</h3><label className="product-photo">{product.photo ? <img src={product.photo} alt="Попередній перегляд товару" /> : <><b>▧</b><span>Додати фото</span></>}<input type="file" accept="image/png,image/jpeg" onChange={(event) => choosePhoto(event.target.files?.[0])} /></label><p>PNG або JPG, до 5 МБ</p><label className="choose-file">Обрати файл<input type="file" accept="image/png,image/jpeg" onChange={(event) => choosePhoto(event.target.files?.[0])} /></label><h3>Попередній перегляд</h3><div className="product-preview-card">{product.photo ? <img src={product.photo} alt="" /> : <i>▧</i>}<div><b>{product.name || "Назва товару"}</b><strong>{product.price || "0.00"} {product.currency}</strong><small>Доступний на {product.terminals.length} терміналах</small></div></div><footer><button type="button" onClick={onCancel}>Скасувати</button><button type="submit">Додати</button></footer></aside></form></section>;
}
