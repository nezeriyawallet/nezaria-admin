...
function BusinessWorkspace({ business, account, onBack, products, onProductsChange }: { business: Business; account: string; onBack: () => void; products: Product[]; onProductsChange: (products: Product[]) => void }) {
  ...
  return <main className="business-workspace">
    ...
    <section className="workspace-shell"><button className="back-businesses" onClick={onBack}>‹ Мої бізнеси</button><div className="workspace-title"><div className="workspace-business-mark">{business.logo ? <img src={business.logo} alt={`Логотип ${business.name}`} style={{ width: "100%", height: "100%", borderRadius: "inherit", objectFit: "cover" }} /> : "▣"}</div><div><h1>{business.name}</h1><p><span className="status-active">Активний</span>{business.ownership} {business.owner && ` ${business.owner}`} · {business.type} · {business.assets?.join(", ") || "USDT"}</p></div><div className="workspace-title-actions"><button onClick={() => setDetailsOpen(true)}>Дані бізнесу</button><button aria-label="Додаткові дії" className="more-button" onClick={() => setMenuOpen(!menuOpen)}>•••</button>{menuOpen && <div className="workspace-popover actions-popover"><button onClick={copyBusinessId}>Копіювати ID</button><button onClick={() => { setMenuOpen(false); notify("Бізнес активний"); }}>Перевірити статус</button></div>}</div></div>
      ...
      </section> : tab === "Товари" ? productAddOpen ? <ProductForm onCancel={() => setProductAddOpen(false)} onAdd={(product) => { onProductsChange([...products, product]); setProductAddOpen(false); notify("Товар додано"); }} /> : <ProductCatalog products={products} onAdd={() => setProductAddOpen(true)} onRemove={(target) => { onProductsChange(products.filter((product) => product !== target)); notify("Товар видалено"); }} onImport={() => notify("Імпорт CSV відкрито")} /> : <section className="workspace-empty"><h2>{tab}</h2><p>{tabActions[tab][1]}</p><button onClick={() => notify(`${tabActions[tab][0]} — функція відкрита`)}>{tabActions[tab][0]}</button></section>}
...
function ProductForm({ onCancel, onAdd }: { onCancel: () => void; onAdd: (product: Product) => void }) {
  const [product, setProduct] = useState<Product>({ name: "", category: "", sku: "", price: "", currency: "USDT", quantity: "100", description: "", terminals: [] });
...
function ProductCatalog({ products, onAdd, onRemove, onImport }: { products: Product[]; onAdd: () => void; onRemove: (product: Product) => void; onImport: () => void }) {
  const [query, setQuery] = useState("");
...
export default function AcquiringPage() {
  ...
  if (view === "dashboard" && selectedBusiness) return <BusinessWorkspace business={selectedBusiness} account={account} onBack={() => setSelectedBusiness(null)} products={productsByBusiness[selectedBusiness.name] || []} onProductsChange={(nextProducts) => changeProducts(selectedBusiness.name, nextProducts)} />;
