...
function MarketingSite({ token, onRefresh, onEnter }: { token: string; onRefresh: () => void; onEnter: () => void }) {
  ...
  return <main className="marketing">
    <header className="marketing-nav"><button className="marketing-logo" type="button" onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })}>NEZERIYA <b>PAY</b></button><nav><a href="#features">Можливості</a><a href="#how">Як це працює</a><a href="#cabinet">Кабінет</a><a href="#developers">Розробникам</a><a href="#pricing">Тариф</a><a href="#faq">FAQ</a></nav><div><button type="button" className="marketing-login" onClick={onEnter}>Увійти</button><button type="button" className="marketing-primary" onClick={onEnter}>Реєстрація</button></div></header>
...
function PaymentsPanel({ payments, onGoTerminals }: { payments: Payment[]; onGoTerminals: () => void }) {
  ...
  const filtered = payments.filter((payment) => (status === "Усі" || payment.status === status) && (source === "Усі" || payment.source === source) && `${payment.id} ${payment.sourceName} ${payment.products.map((item) => item.name).join(" ")}`.toLocaleLowerCase("uk-UA").includes(query.trim().toLocaleLowerCase("uk-UA")));
  ...
  return <section className="payments-view"><header className="payments-heading"><h2>Платежі</h2><p>{payments.length} чеків із терміналів, платіжних посилань і сайту</p></header><div className="payments-layout"><div><div className="payments-tools"><label>⌕<input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Пошук за ID, товаром або хешем" /></label><select value={status} onChange={(event) => setStatus(event.target.value)}><option>Усі</option><option>Оплачено</option><option>Очікує підтвердження</option><option>Недоплата</option></select><select value={source} onChange={(event) => setSource(event.target.value)}><option>Усі</option><option>Термінал</option><option>Платіжне посилання</option><option>Сайт</option></select></div><div className="payments-table"><div className="payments-table-head"><span>Дата і час</span><span>Платіж</span><span>Джерело</span><span>Сума</span><span>Статус</span></div>{filtered.map((payment) => <button type="button" className={selected?.id === payment.id ? "receipt-row selected" : "receipt-row"} key={payment.id} onClick={() => setSelected(payment)}><span><b>{formatDate(payment.createdAt).split(",")[0]}</b><small>{formatDate(payment.createdAt).split(",")[1]?.trim()}</small></span><span><b>{total(payment)}</b><small>{payment.id} · {payment.products.length} {payment.products.length === 1 ? "товар" : "товари"}</small></span><span>{payment.source === "Термінал" ? "▦" : payment.source === "Сайт" ? "⌘" : "↗"} {payment.sourceName}</span><strong>+{payment.amount} {payment.currency}</strong><em className={payment.status === "Оплачено" ? "paid" : payment.status === "Недоплата" ? "underpaid" : "pending"}>{payment.status}</em></button>)}{!filtered.length && <p className="payments-no-results">Чеків за цими фільтрами немає.</p>}</div></div>{selected && <aside className="receipt-detail"><button type="button" aria-label="Закрити деталі платежу" onClick={() => setSelected(null)}>×</button><h2>Платіж {selected.id}</h2><p><em className={selected.status === "Оплачено" ? "paid" : selected.status === "Недоплата" ? "underpaid" : "pending"}>{selected.status}</em> {formatDate(selected.createdAt)}</p><strong>{selected.amount} {selected.currency}</strong><small>Джерело</small><h3>{selected.source === "Термінал" ? "▦" : selected.source === "Сайт" ? "⌘" : "↗"} {selected.sourceName}</h3><div className="receipt-products"><b>Товари ({selected.products.length})</b>{selected.products.map((item, index) => <article key={`${item.name}-${index}`}>{item.photo ? <img src={item.photo} alt="" /> : <i>▧</i>}<span><strong>{item.name}</strong><small>× {item.quantity}</small></span><b>{item.price} {item.currency}</b></article>)}</div><dl><div><dt>Разом</dt><dd>{selected.amount} {selected.currency}</dd></div><div><dt>Комісія 0,5%</dt><dd>−{(Number(selected.amount) * .005).toFixed(2)} {selected.currency}</dd></div><div><dt>Зараховано на баланс</dt><dd>{(Number(selected.amount) * .995).toFixed(2)} {selected.currency}</dd></div></dl>{selected.transaction && <footer><small>Транзакція</small><b>{selected.transaction}</b>{selected.wallet && <><small>Гаманець клієнта</small><b>{selected.wallet}</b></>}</footer>}</aside>}</div></section>;
}
