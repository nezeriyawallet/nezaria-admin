...
function UsersPanel({ walletMetrics, users }: { walletMetrics: WalletMetrics | null; users: WalletUser[] }) {
  ...
  const highestNrzBalance = users.length ? Math.max(...users.map((user) => Number(user.nzrPoints) || 0)) : null;
