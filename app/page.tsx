...
function MediaScreen({ onBack }: { onBack: () => void }) {
  ...
  const [view, setView] = useState<"banners" | "salary">("banners");
  ...
  useEffect(() => { void load(); void loadPayouts(); }, []);
...
function MediaOwnerPanel() {
  ...
  const load = async () => { const [media, paymentData] = await Promise.all([fetch("/api/owner/media", { headers: headers() }), fetch("/api/owner/media/payouts", { headers: headers() })]); if (media.ok) setData(await media.json()); if (paymentData.ok) { const result = await paymentData.json(); setPayouts(result.payouts || []); } };
  useEffect(() => { void load(); }, []);
