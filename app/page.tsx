...
export default function Home() {
  ...
  const financeMetricsQuery = (month = financeMonth, turnoverYear = financeTurnoverYear) => [financeMonthQuery(month), `turnoverYear=${turnoverYear}`].filter(Boolean).join("&");

  const loadWalletMetrics = async (month = financeMonth, turnoverYear = financeTurnoverYear) => {
    ...
    const query = [financeMetricsQuery(month, turnoverYear), `_=${Date.now()}`].filter(Boolean).join("&");
    try {
  ...
  }, [accessRole, financeMonth, financeTurnoverYear]);
  ...
  useEffect(() => {
    ...
    const ownerSession = window.sessionStorage.getItem("nezeriya_owner_session") || window.localStorage.getItem("nezeriya_owner_session");
    if (!token || !ownerSession) return;
    void fetch("/api/owner/users", { cache: "no-store", headers: { Authorization: `Bearer ${token}`, "x-owner-session": ownerSession } })
  ...
  const refreshUsers = async (showNotice = false) => {
    ...
    try {
      ...
      const [metricsResponse, usersResponse] = await Promise.all([
        fetch(`/api/owner/metrics?${[financeMetricsQuery(), `_=${Date.now()}`].filter(Boolean).join("&")}`, { cache: "no-store", headers }),
      ...
      if (!metricsResponse.ok || !usersResponse.ok) throw new Error();
  ...
  useEffect(() => {
    ...
    const refreshMetricsOnly = async () => {
      ...
      if (!token || !ownerSession) return;
      try {
        const response = await fetch(`/api/owner/metrics?${[financeMetricsQuery(), `_=${Date.now()}`].filter(Boolean).join("&")}`, { cache: "no-store", headers: { Authorization: `Bearer ${token}`, "x-owner-session": ownerSession } });
        const body = await response.json().catch(() => ({}));
  ...
  }, [accessRole, active, financeMonth, financeTurnoverYear]);
  ...
  const totalIncome = walletMetrics ? displayMetric(metricNumber(walletMetrics.totalCommission) + metricNumber(walletMetrics.nzrExchangeNetUsd), "$") : "—";
      ...
      <section className="content">
        ...
        <div className="dashboard">
          ...
          <section className="main-grid">
            <article className="panel activity-panel activity-panel-wide"><div className="panel-head"><div><p className="panel-label">ЖИВА АКТИВНІСТЬ</p><h2>Зараз у гаманці</h2></div><span className="live"><i /> LIVE</span></div><div className="live-count">{walletOnline}<span> онлайн</span></div><p className="subtle">Максимум одночасно онлайн за кожен день.</p><div className="activity-bars" aria-label="Денні піки онлайн">{dailyOnlinePeaks.length ? dailyOnlinePeaks.map((item) => { const peak = metricNumber(item.peakOnline); const label = `${formatOnlinePeakDate(item.date)} — максимум ${peak} онлайн`; return <i key={item.date} title={label} aria-label={label} tabIndex={0} onMouseEnter={() => setHoveredOnlinePeak(item)} onMouseLeave={() => setHoveredOnlinePeak(null)} onFocus={() => setHoveredOnlinePeak(item)} onBlur={() => setHoveredOnlinePeak(null)} style={{ height: `${Math.max(4, Math.round(peak / maximumDailyOnline * 100))}%` }} />; }) : <span className="online-chart-empty">Дані з’являться після першої активної сесії.</span>}</div><div className="online-peak-tooltip" role="status">{hoveredOnlinePeak ? <><b>{formatOnlinePeakDate(hoveredOnlinePeak.date)}</b><span>Максимум: {displayMetric(hoveredOnlinePeak.peakOnline)} онлайн</span><small>Пікова кількість одночасно активних гаманців.</small></> : <span>Наведіть на стовпчик, щоб побачити статистику.</span>}</div><div className="legend"><span><i className="mint-dot" /> Денний максимум</span><span><i className="gray-dot" /> Дані зберігаються щодня</span></div></article>
            <article className="panel earnings-panel earnings-panel-compact"><div className="panel-head"><div><p className="panel-label">ЗАРОБІТОК</p><h2>Загальна сума доходу</h2></div><button className="dots" aria-label="Додаткові дії">•••</button></div><div className="chart-summary"><strong>{totalIncome}</strong></div><p className="income-note">Комісії + Stars за курсом Fragment − успішні викупи NZR.</p><span className="earnings-period">За весь час</span></article>
          </section>
...
function FinancePanel({ walletMetrics, selectedMonth, selectedTurnoverYear, onSelectedMonthChange, onSelectedTurnoverYearChange }: { walletMetrics: WalletMetrics | null; selectedMonth: string; selectedTurnoverYear: number; onSelectedMonthChange: (month: string) => void; onSelectedTurnoverYearChange: (year: number) => void }) {
  ...
  const companyTurnoverUsd = walletMetrics ? metricNumber(walletMetrics.companyTurnoverUsd) : 0;
  const monthlyCompanyTurnoverUsd = walletMetrics ? metricNumber(walletMetrics.monthlyCompanyTurnoverUsd) : 0;
  const yearlyCompanyTurnoverUsd = walletMetrics ? metricNumber(walletMetrics.yearlyCompanyTurnoverUsd) : 0;
  ...
  return <section className="finance-page">
    <section className="heading-row"><div><p className="eyebrow">ФІНАНСИ</p><h1>Фінансова <span>аналітика</span></h1><p className="subtle">Дані синхронізуються з Nezeriya Wallet API.</p></div><div className="heading-actions finance-heading-actions"><label className="month-picker" htmlFor="finance-month"><span>Перевірити місяць</span><input id="finance-month" type="month" value={selectedMonth} max={new Date().toISOString().slice(0, 7)} onChange={(event) => onSelectedMonthChange(event.target.value)} /></label><label className="month-picker" htmlFor="finance-turnover-year"><span>Перевірити рік</span><select id="finance-turnover-year" value={selectedTurnoverYear} onChange={(event) => onSelectedTurnoverYearChange(Number(event.target.value))}>{Array.from({ length: Math.max(1, new Date().getFullYear() - 2020 + 1) }, (_, index) => new Date().getFullYear() - index).map((year) => <option key={year} value={year}>{year} р.</option>)}</select></label><span className="live"><i /> LIVE</span></div></section>
    <section className="finance-highlight"><article className="panel"><p>Загальна сума доходу</p><strong>{totalIncome === null ? "—" : displayMetric(totalIncome, "$")}</strong><span>За весь час · комісії + Fragment-виплата Stars + обмін NZR</span></article><article className="panel finance-turnover"><p>Загальний оборот компанії</p><strong>{walletMetrics ? displayMetric(companyTurnoverUsd, "$") : "—"}</strong><span>За весь час · Stars, продаж NZR і виплати за викуп</span></article><article className="panel"><p>Річний оборот</p><strong>{walletMetrics ? displayMetric(yearlyCompanyTurnoverUsd, "$") : "—"}</strong><span>{selectedTurnoverYear} р. · усі грошові потоки обміну</span></article><article className="panel"><p>Оборот за місяць</p><strong>{walletMetrics ? displayMetric(monthlyCompanyTurnoverUsd, "$") : "—"}</strong><span>{selectedMonthName} · усі грошові потоки обміну</span></article><article className="panel"><p>Чистий прибуток за місяць</p><strong>{walletMetrics ? displayMetric(monthlyExchangeNetUsd, "$") : "—"}</strong><span>{selectedMonthName} · Stars і продаж NZR мінус викуп NZR</span></article><article className="panel"><p>Загальний дохід за місяць</p><strong>{monthlyTotalIncome === null ? "—" : displayMetric(monthlyTotalIncome, "$")}</strong><span>{selectedMonthName} · чистий прибуток + комісії</span></article></section>
...
function AudienceRevenueCards({ metrics, users }: { metrics: WalletMetrics | null; users: WalletUser[] }) {
...
function CountryStats({ users }: { users: WalletUser[] }) {
  ...
  return <article className="panel country-summary"><p>Географія реєстрацій</p><strong>{confirmed.length}</strong><span>Підтверджено номерів</span>{countries.length ? <div className="country-list">{countries.map((country) => <span key={country.code}><b>{country.flag} {country.name}</b><strong>{country.count}</strong></span>)}</div> : <p className="users-empty">Країни з’являться після підтвердження контакту.</p>}</article>;
}
...
function WinsPanel({ wins, summary, refreshing, onRefresh }: { wins: WheelWin[]; summary: WheelWinSummary; refreshing: boolean; onRefresh: () => void }) {
  return <section className="users-page">
    ...
    <section className="users-summary"><article className="panel users-primary"><p>Виграли в іграх за місяць</p><strong>{displayMetric(summary.monthlyWonNzr)} NZR</strong><span>Рулетка: {displayMetric(summary.monthlyWheelWonNzr)} · Plinko: {displayMetric(summary.monthlyPlinkoWonNzr)}</span></article><article className="panel"><p>Зібрано ставок за місяць</p><strong>{displayMetric(summary.monthlyCollectedNzr)} NZR</strong><span>Рулетка: {displayMetric(summary.monthlyWheelSpentNzr)} · Plinko: {displayMetric(summary.monthlyPlinkoSpentNzr)}</span></article><article className="panel"><p>Чистий заробіток за місяць</p><strong>{displayMetric(summary.monthlyNetEarningsNzr)} NZR</strong><span>Зібрано ставок мінус виплачені виграші</span></article></section>
    <article className="panel users-table-panel"><div className="panel-head"><div><p className="panel-label">ІСТОРІЯ ІГОР</p><h2>Останні результати рулетки та Plinko</h2></div><span>{wins.length} результатів показано</span></div>{wins.length === 0 ? <p className="users-empty">Результатів поки немає. Нові ігри з&apos;являться тут після завершення.</p> : <div className="users-table-wrap"><table className="users-table"><thead><tr><th>Дата й час</th><th>Telegram ID</th><th>Користувач</th><th>Wallet ID</th><th>Що випало</th><th>Що отримав</th></tr></thead><tbody>{wins.map((win, index) => <tr key={`${win.id}-${win.walletId}-${index}`}><td>{new Date(win.createdAt).toLocaleString("uk-UA", { timeZone: "Europe/Kyiv", day: "2-digit", month: "2-digit", year: "numeric", hour: "2-digit", minute: "2-digit", second: "2-digit", hour12: false })}</td><td>{win.id}</td><td><strong>{win.username ? `@${win.username}` : win.name || "Без username"}</strong>{win.name && win.username && <small>{win.name}</small>}</td><td>{win.walletId}</td><td>{win.dropped}</td><td className="nzr-points">{win.reward}</td></tr>)}</tbody></table></div>}</article>
  </section>;
}
