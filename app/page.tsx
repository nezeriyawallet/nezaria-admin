...
function NotificationBell({ role, owner }: { role: "owner" | "worker" | "media"; owner?: boolean }) {
  ...
  const markOneRead = (id: string) => { const ids = [...new Set([...seen, id])]; setSeen(ids); window.localStorage.setItem(key, JSON.stringify(ids)); setContext(null); };
...
function AccountMenu({ name, role, avatar, onSignOut, onNicknameChange }: { name: string; role: string; avatar?: string | null; onSignOut: () => void; onNicknameChange?: (value: string) => void }) {
  ...
  useEffect(() => {
    ...
    const attachContextMenu = () => { popup.querySelectorAll("article").forEach((article, index) => { article.addEventListener("contextmenu", (event) => { event.preventDefault(); const item = items[index]; if (!item) return; contextMenu.style.left = `${event.clientX}px`; contextMenu.style.top = `${event.clientY}px`; contextMenu.hidden = false; contextMenu.onclick = () => { const ids = [...new Set([...seen(), item.id])]; window.localStorage.setItem(cacheKey, JSON.stringify(ids)); contextMenu.hidden = true; render(); attachContextMenu(); }; }); }); };
    const load = async () => { const response = await fetch(`/api/notifications?role=${notificationRole}`, { headers: { Authorization: `Bearer ${token}` } }); if (response.ok) { const result = await response.json(); items = result.notifications || []; render(); attachContextMenu(); } };
